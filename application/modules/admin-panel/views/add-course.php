    ...
    <div class="admin-main">
        <div class="admin-sub-main">
            ...
            <div class="form-group">
                <h2 class="add-header">Add Course</h2>
                <form method="post" class="form-input" id="add_course_form" enctype="multipart/form-data">
                    <div class="input-container">
                        <!-- input 1 -->
                        <div class="input-sub-contain">
                            <label class="input-label-name">Course Name</label>
                            <input type="text" class="form-text-input" name="course_name" id="course_name">
                            <div id="course_name_error" class="error"></div>
                        </div>
                        <!-- input 2 -->
                        <div class="input-sub-contain">
                            <label class="input-label-name">Course Offered By</label>
                            <input type="text" class="form-text-input" name="course_offered_by" id="course_offered_by">
                            <div id="course_offered_by_error" class="error"></div>
                        </div>
                    </div>
                    <div class="input-container mt-2">
                        <!-- input 1 -->
                        <div class="input-sub-contain">
                            <label class="input-label-name">Course Instructor</label>
                            <input type="text" class="form-text-input" name="course_instructor" id="course_instructor">
                            <div id="course_instructor_error" class="error"></div>
                        </div>
                        <!-- input 2 -->
                        <div class="input-sub-contain">
                            <label class="input-label-name">Course Duration</label>
                            <input type="text" class="form-text-input" name="course_duration" id="course_duration">
                            <div id="course_duration_error" class="error"></div>
                        </div>
                    </div>
                    <div class="input-container mt-2">
                        <!-- input 1 -->
                        <div class="input-sub-contain">
                            <label class="input-label-name">Course Description</label>
                            ...
                            <div id="course_description_error" class="error"></div>
                        </div>
                    ...
                    <!-- <div class="input-container mt-5">
                        input 1
                        <div class="input-sub-contain">
                            <label class="input-label-name">Course Image</label>
                            <div class="upload-btn-wrapper">
                                <button class="btn">Upload Image</button>
                                <input type="file" name="course_image" id="course_image">
                            </div>
                            <div id="course_image_error" class="error"></div>
                        </div>
                    </div> -->
                    <div class="button-submit">
                        <input type="submit" value="Add Course" class="add-course-btn" name="add_course"
                            id="add_course">
                    </div>
                </form>
...
<script>
    $(document).ready(function () {
        ...
        $('#add_course_form').on('submit', function (event) {
            ...
            // Validate the course name input field
            ...
            // Validate the course offered by input field
            var courseOfferedBy = $('#course_offered_by').val();
            if (courseOfferedBy.length == 0) {
                $('#course_offered_by_error').html('Please enter the course offered by');
                return false;
            } else {
                $('#course_offered_by_error').html('');
            }

            // Validate the course instructor input field
            var courseInstructor = $('#course_instructor').val();
            if (courseInstructor.length == 0) {
                $('#course_instructor_error').html('Please enter the course instructor');
                return false;
            } else {
                $('#course_instructor_error').html('');
            }

            // Validate the course duration input field
            var courseDuration = $('#course_duration').val();
            if (courseDuration.length == 0) {
                $('#course_duration_error').html('Please enter the course duration');
                return false;
            } else {
                $('#course_duration_error').html('');
            }

            // Validate the course description input field
            var courseDescription = $('#course_description').val();
            if (courseDescription.length == 0) {
                $('#course_description_error').html('Please enter the course description');
                return false;
            } else {
                $('#course_description_error').html('');
            }

            // // Validate the course image input field
            // var courseImage = $('#course_image').val();
            // if (courseImage.length == 0) {
            //     $('#course_image_error').html('Please upload an image for the course');
            //     return false;
            // } else {
            //     $('#course_image_error').html('');
            // }

            // If all input fields are valid, send an AJAX request to the server to add the course
            $.ajax({
                type: 'post',
                url: '<?php echo base_url(); ?>index.php/admin/add',
                data: formData,
                processData: false,
                contentType: false,
